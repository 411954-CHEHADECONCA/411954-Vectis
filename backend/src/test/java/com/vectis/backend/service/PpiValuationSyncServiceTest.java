...
class PpiValuationSyncServiceTest {
    ...
    void syncPpiValuations_createsValuation_whenPriceAvailable() {
        ...
        given(valuationRepository.findByInvestmentAsset_IdAndValuationDate(eq(assetId), any(LocalDate.class)))
    ...
    @Test
    @DisplayName("syncPpiValuations no crea valuación si ya existe para el día y es manual (idempotencia)")
    void syncPpiValuations_skipsAsset_whenValuationAlreadyExistsToday() {
        ...
        InvestmentValuation existing = InvestmentValuation.builder()
                ...
                .valuationDate(LocalDate.of(2026, 1, 16))
                .pricePerUnit(new BigDecimal("1400.0000"))
                .source("MANUAL")
                .autoGenerated(false)
                .build();
        ...
        verify(valuationRepository, never()).save(any());
        assertThat(existing.getPricePerUnit()).isEqualByComparingTo("1400.0000");
    }

    @Test
    @DisplayName("savePpiValuation pisa una valuación auto-generada existente con el precio de mercado")
    void savePpiValuation_overridesAutoGeneratedExisting() {
        UUID assetId = UUID.randomUUID();
        InvestmentAsset asset = buildBonoAsset(assetId, "AL30");
        ...
        InvestmentValuation existing = InvestmentValuation.builder()
                ...
                .investmentAsset(asset)
                .valuationDate(date)
                .pricePerUnit(new BigDecimal("1000.0000"))
                .source("MANUAL")
                .autoGenerated(true)
                .build();
        ...
        service.savePpiValuation(asset, date, new BigDecimal("1450.5000"));

        assertThat(existing.getPricePerUnit()).isEqualByComparingTo("1450.5000");
        assertThat(existing.getSource()).isEqualTo("PPI");
        assertThat(existing.isAutoGenerated()).isTrue();
        verify(valuationRepository).save(existing);
    }

    @Test
    @DisplayName("savePpiValuation NO pisa una valuación manual (autoGenerated=false) existente")
    void savePpiValuation_doesNotOverrideManualExisting() {
        UUID assetId = UUID.randomUUID();
        InvestmentAsset asset = buildBonoAsset(assetId, "AL30");
        LocalDate date = LocalDate.of(2026, 7, 23);
        ...
        assertThat(existing.getPricePerUnit()).isEqualByComparingTo("1000.0000");
