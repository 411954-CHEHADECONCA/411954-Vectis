...
export class InversionesComponent implements OnInit {
  ...
  calcTramosCP(asset: InvestmentResponse): FCITramoCP[] {
    ...
    // Las valuaciones se guardan a diario (backfill + sync), pero los tramos se muestran por MES:
    // se toma la última valuación de cada mes calendario (cierre de fin de mes) como límite, y se
    // excluye el mes en curso (ese tramo lo cubre el "en curso" hasta hoy). Así una LETRA/BONO/FCI
    // con seguimiento muestra ~1 tramo por mes en vez de uno por día. Las valuaciones diarias siguen
    // disponibles para el valor actual (calcValorActualCP) y para el precio de operaciones.
    const todayIso        = this.todayIso();
    const currentMonthKey = todayIso.slice(0, 7); // 'YYYY-MM'
    const monthlyValuations = [...(asset.valuations ?? [])]
      // Excluir el mes en curso (lo cubre el "en curso") y cualquier valuación con fecha futura
      // (no representa ganancia realizada: evita tramos "completados" adelantados).
      .filter(v => v.valuationDate.slice(0, 7) !== currentMonthKey && v.valuationDate <= todayIso)
      ...
      .reduce((byMonth, v) => {
        ...
        return byMonth;
      }, new Map<string, NonNullable<InvestmentResponse['valuations']>[number]>());

    // Cada valuación mensual, suscripción, rescate y revalúo define un límite de tramo. El precio de
    // mercado de una suscripción/rescate es su precio implícito (monto / nominales) en esa fecha.
    const events: CPEvent[] = [
      ...[...monthlyValuations.values()].map(v => ({
